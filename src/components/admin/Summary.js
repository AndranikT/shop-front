...
const Summary = () => {
    ...
    useEffect(()=>{
        async function fetchData(){
            try{
                const res = await axios.get(`${url}/orders/stats`,setHeaders()).then((result)=>{
                    result.data.sort(compare)
                    setOrders(result.data)
                    setOrdersPerc(result.data[0]?.total)
                })
                return res?.data
            }catch (err){
                console.log(err)
    ...
    useEffect(()=>{
        async function fetchData(){
            try{
                const res = await axios.get(`${url}/orders/income/stats`,setHeaders()).then((result)=>{
                result.data.sort(compare)
                setIncome(result.data)
                setIncomePerc(result.data[0]?.total / 1000)
                })
                return res?.data
            }catch(err){
                console.log(err)
    ...
    const data = [
        ...
        },
        {
            ...
            isMoney : true,
            title : "Earnings",
            color : "rgb(253, 181, 400 )",
            bgColor : "rgba(253, 181, 400, 0.12)",
